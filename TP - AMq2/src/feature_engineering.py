...
class FeatureEngineeringPipeline():
    ...
    '''
    Class for the Feature Engineering Pipeline
    '''
    ...
    def data_transformation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        Method for transforming and cleaning the DataSet    
        """
        ...
        df_transformed['Item_Fat_Content'] = \
            df_transformed['Item_Fat_Content']\
                .replace({'low fat':  'Low Fat', 'LF': 'Low Fat', 'reg': 'Regular'})

        # LIMPIEZA: faltantes en el peso de los productos
        productos = list(df_transformed[df_transformed['Item_Weight']\
                                        .isnull()]['Item_Identifier'].unique())
        for producto in productos:
            moda = (df_transformed[df_transformed['Item_Identifier']==\
                                   producto][['Item_Weight']]).mode().iloc[0,0]
            df_transformed.loc[df_transformed['Item_Identifier']==\
                               producto, 'Item_Weight'] = moda

        # LIMPIEZA: faltantes en el tamaño de las tiendas
        outlets = list(df_transformed[df_transformed['Outlet_Size']\
                                      .isnull()]['Outlet_Identifier'].unique())
        ...
        df_transformed.loc[df_transformed['Item_Type'] == \
                           'Health and Hygiene', 'Item_Fat_Content'] = 'NA'
        ...
        df_transformed.loc[df_transformed['Item_Type'] ==\
                            'Fruits and Vegetables', 'Item_Fat_Content'] = 'NA'

        # FEATURES ENGINEERING: creando categorías para 'Item_Type'
        df_transformed['Item_Type'] = \
            df_transformed['Item_Type'].replace({'Others': 'Non perishable',
        'Health and Hygiene': 'Non perishable', 'Household': 'Non perishable',
        'Seafood': 'Meats', 'Meat': 'Meats', 'Baking Goods': 'Processed Foods',
        'Frozen Foods': 'Processed Foods', 'Canned': 'Processed Foods', 'Snack Foods':
        'Processed Foods', 'Breads': 'Starchy Foods', 'Breakfast': 'Starchy Foods',
        ...
        df_transformed['Item_MRP'] =\
              pd.qcut(df_transformed['Item_MRP'], 4, labels = [1, 2, 3, 4])
        ...
        df_transformed['Outlet_Size'] = \
            df_transformed['Outlet_Size'].replace({'High': 2, 'Medium': 1, 'Small': 0})
        # Estas categorias se ordenaron asumiendo la categoria 2 como más lejos
        df_transformed['Outlet_Location_Type'] = \
            df_transformed['Outlet_Location_Type'].replace({'Tier 1': 2, 'Tier 2': 1, 'Tier 3': 0})
